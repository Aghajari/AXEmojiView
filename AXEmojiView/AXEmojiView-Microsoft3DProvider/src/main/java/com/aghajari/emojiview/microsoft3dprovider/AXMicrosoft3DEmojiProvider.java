...
package com.aghajari.emojiview.microsoft3dprovider;
...
public final class AXMicrosoft3DEmojiProvider extends AXPresetEmojiProvider {

    public AXMicrosoft3DEmojiProvider(Context context) {
    ...
    public AXMicrosoft3DEmojiProvider(Context context, int[] icons) {
        super(context, icons);
    }

    @Override
    public EmojiData getEmojiData() {
        return AXMicrosoft3DEmojiData.instance;
    }

    @Override
    protected EmojiCategory createCategory(int i, int icon) {
        return new AXMicrosoft3DEmojiCategory(i, icon, getEmojiData());
    }

    public static class AXMicrosoft3DEmojiCategory extends AXPresetEmojiCategory {
        public AXMicrosoft3DEmojiCategory(int i, int icon, EmojiData emojiData) {
            super(i, icon, emojiData);
        }

        @Override
        protected Emoji createEmoji(String code, int category, int index, EmojiData emojiData){
            return new AXMicrosoft3DEmoji(code, emojiData);
        }
    }

    public static class AXMicrosoft3DEmoji extends AXPresetEmoji {
        public AXMicrosoft3DEmoji(String code, EmojiData emojiData) {
