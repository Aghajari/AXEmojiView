...
public abstract class AXPresetEmojiReplacer implements EmojiReplacer {
    ...
    public void replaceWithImages(final Context context, final View view, Spannable text, float emojiSize, Paint.FontMetrics fontMetrics) {
        ...
        final AXPresetEmojiLoader.EmojiSpan[] existingSpans = text.getSpans(0, text.length(), AXPresetEmojiLoader.EmojiSpan.class);
        ...
        if (existingSpanPositions.size() == 0) {
            AXPresetEmojiLoader.replaceEmoji(text, fontMetrics, (int) emojiSize, false);
        } else {
            ...
            for (int i = 0; i < findAllEmojis.size(); i++) {
                ...
                if (!existingSpanPositions.contains(location.start)) {
                    List<AXPresetEmojiLoader.SpanLocation> list = new ArrayList<>();
                    AXPresetEmojiLoader.replaceEmoji(location.emoji.getUnicode(), fontMetrics, (int) emojiSize, false, list);
                    for (AXPresetEmojiLoader.SpanLocation l : list) {
