...
public class Main {
    ...
    public static void sort(int i, List<EmojiInfo> list) {
        list.sort((o1, o2) -> {
            int a = indexOf(EmojiInfo.data.get(i), o1);
            int b = indexOf(EmojiInfo.data.get(i), o2);
            if (a == -1)
                return b == -1 ? 0 : 1;
            else if (b == -1)
                return -1;
    ...
    public static void save(EmojiFileFinder finder) {
        ...
        for (int i = 0; i < dataPlatform.size(); i++) {
            for (int j = 0; j < dataPlatform.get(i).size(); j++) {
                ...
                finder.save(dataPlatform.get(i).get(j),
                        platform + "_emoji/" + i + "_" + j + ".png");
            }
        }
    }

    public interface EmojiFileFinder {
        void save(EmojiInfo emoji, String output);
    }

    public static void downloadEmoji(EmojiInfo emoji, String outputPath) {
        //if (true) return;
        if (new File(platform).exists())
            return;

        String link = emoji.image;
        try {
